...
export default class Refund extends BaseModel {
  ...
  static async removeRelation(refund: Refund) {
    ...
    // if (refund.$dirty.deletedAt && refund.receiptId) {
    //   const receipt = await Receipt.findOrFail(refund.receiptId)
    //   const disk = drive.use()
    ...
    if (refund.$dirty.deletedAt) {
      ...
      if (!refund.receipt) {
        console.log('No receipt found for this refund')
        return
      }

      const disk = drive.use()
      await Promise.all([disk.delete(refund.receipt.path), refund.receipt.delete()])
    }
  }
}
